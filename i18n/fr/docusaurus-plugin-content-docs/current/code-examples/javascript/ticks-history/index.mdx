---
title: Historique des tiques
draft: false
sidebar_label: Historique des ticks
sidebar_position: 2
tags:
  - code
  - exemple
  - ticks
  - historique
keywords:
  - code
  - exemple
  - ticks
  - historique
description: Abonnez-vous à la fonction d'historique des ticks de Deriv API pour obtenir les heures et les prix des ticks précédents. Intégrez cette fonctionnalité dans votre application de trading grâce à cet exemple d'API.
---

import RenderOfficialContents from '@site/src/components/RenderOfficialContents';
import SandboxIframe from '@site/src/components/SandboxIframe';
import { sandboxes } from '@site/src/components/SandboxIframe/utility/sandboxes';

L'historique des ticks vous permet d'obtenir une collection d'heures et de prix de ticks passés.

<SandboxIframe sandbox={sandboxes.ticks_history}/>

<RenderOfficialContents>
  <p>Nous nous abonnons à la fonctionnalité ticks de l'API websocket Deriv pour suivre les ticks actuels et passés.</p>
</RenderOfficialContents>

Vous pouvez par exemple créer un graphique de ticks interactif.
