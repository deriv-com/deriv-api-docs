...
귀하의 사용자를 인증하기 위해서는 [대시보드](/대시보드) 페이지 및 OAuth details 항목의 Register application탭에서 OAuth Redirect URL로 사용될 특정 URL을 지정하세요. 그런 다음 웹사이트나 앱에 로그인 버튼을 추가하고 사용자를 \*\*`https://oauth.deriv.com/oauth2/authorize?app_id=your_app_id`\*\*로 안내합니다(여기서 your_app_id는 앱의 ID).
